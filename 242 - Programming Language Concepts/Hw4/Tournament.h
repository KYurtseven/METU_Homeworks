...
public:
	...
	void refresh(std::vector<Pokemon *>& pokemons);
	Trainer* bigDuel(Trainer* trainer1, Trainer* trainer2);
    // Getters

    const std::string& getTournamentName() const;

	// Own Methods
	//void addtovector(Trainer trainer);
private:
	...
	int duelBetweenP(Pokemon *pokemon1, Pokemon *pokemon2, Arena currentArena);
	void duelBetween_helper(Pokemon *pokemon1, Pokemon *pokemon2, Arena currentArena);
	// Own Attributes
